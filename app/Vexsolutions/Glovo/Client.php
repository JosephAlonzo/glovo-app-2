...
class Client {
    ...
    private static function build_request( $request ) {
        if (!extension_loaded( "curl" )) {
            throw new VexGlovoException( "cURL extension not found. You need to enable cURL in your php.ini or another configuration you have." );
        }

        if (!isset( $request["method"] )) {
            throw new VexGlovoException( "No HTTP METHOD specified" );
        }

        if (!isset( $request["auth"] )) {
            throw new VexGlovoException( "No auth specified" );
        }

        if (!isset( $request["uri"] )) {
            throw new VexGlovoException( "No URI specified" );
        }
        ...
        if (isset( $request["data"] )) {
           ...
           // throw new \Exception($request["data"], "2001");

            if (function_exists( 'json_last_error' )) {
                $json_error = json_last_error();
                if ($json_error != JSON_ERROR_NONE) {
                    throw new VexGlovoException( "JSON Error [{$json_error}] - Data: " . $request["data"] );
                }
            }
    ...
    public static function exec( $request ) {
        ...
        if ($api_result === false) {
            throw new VexGlovoException ( curl_error( $connect ) );
        }
        ...
        if ($response['status'] != 200) {
            $message = isset($response['response']['error']['message']) ? $response['response']['error']['message'] : $response['response']['error'];
                throw new VexGlovoException ( $message, $response['status'] );
        }
